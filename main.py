...
def load_args() -> tuple[argparse.ArgumentParser, argparse.Namespace]:
    ...
    parser.add_argument('--msg','-m', type=str, help='Direct question',default="Read the gitdiff messages and create a git commit message")
    parser.add_argument('--model','-md', type=str, help='Model to use', default="magistral")
    parser.add_argument('--system','-s', type=str, help='pass a prompt name ',default="git-expert")
    ...
    parser.add_argument('--task','-t', type=str, help='name of the template inside prompt_templates',default="commit.md")
...
def print_initial_info(prog:Program) -> None:
    ...
    system_p_file_path = prog.config.get('SYSTEM_PROMPT_FILE', '') # Provide a default empty string if key is missing
    system_p_file :str = system_p_file_path.split("/")[-1].replace('.md','').replace('_'," ")
