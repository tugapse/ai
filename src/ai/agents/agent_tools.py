...
def send_notification(**kwargs) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "FAILED", "error": "notify-send not found. Install 'libnotify'."}
...
def _sanitize_output_path(full_path: str) -> str:
    ...
    try:
        ...
        if full_path.startswith(PROJECT_ROOT):
            ...
            return "@ROOT" if relative == "." else f"@ROOT/{relative.replace(os.sep, '/')}"
...
def execute_command(**kwargs) -> Dict[str, Any]:
    ...
    if not command: return {"status": "FAILED", "error": "No command provided."}
...
AVAILABLE_TOOLS = {
    ...
    "execute_command": execute_command, "send_notification": send_notification
}