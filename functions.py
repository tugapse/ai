...
def get_files(directory, extension=None) -> list[ContextFile]:
    ...
    search_pattern = f"*.{extension}" if extension and not extension.startswith('.') else f"*{extension}" if extension else "*"
    ...
    for filepath in base_path.rglob(search_pattern): # rglob is recursive glob
        if filepath.is_file():
            file_list.append(ContextFile(filename=str(filepath.resolve()))) # Ensure absolute path and string
...
def read_file(filename):
    ...
    try:
        return file_path.read_text(encoding="utf-8") # Specify encoding for robust reading
...
def get_root_directory() -> str:
    ...
    log(f"WARNING: '{ProgramSetting.ROOT_DIRECTORY}' not found in config. Using fallback: {fallback_path}", level="WARNING")
...
def ensure_directory_exists(path: str):
    ...
    if not os.path.exists(path):
        try:
            ...
            log(f"Created directory: {path}", level="INFO")
        except OSError as e:
            log(f"Error creating directory {path}: {e}", level="ERROR")
            out(f"{Color.RED}Error: Failed to create directory: {path} - {e}{Color.RESET}")
            sys.exit(1) # Exit on critical directory creation failure
    else:
        log(f"Directory already exists: {path}", level="DEBUG")
...
def log(text, start_line="[ * ]", level="INFO", **kargs): # Added level for consistency
    ...
    if ProgramConfig.current and ProgramConfig.current.get(ProgramSetting.PRINT_LOG, True) and not LOCK_LOG:
        ...
        elif level == "DEBUG": # Debug is now handled by its own func, but keeping consistency here
...
def debug(text, start_line="[ # ]", **kargs):
    ...
    """
    if ProgramConfig.current and ProgramConfig.current.get(ProgramSetting.PRINT_DEBUG, False) and not LOCK_LOG:
...
def out(text, **kargs):
    ...
    """
    if ProgramConfig.current and ProgramConfig.current.get(ProgramSetting.PRINT_OUTPUT, True):
        print(text, **kargs)
        sys.stdout.flush() # Ensure it's flushed immediately
